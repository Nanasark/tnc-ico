...
// File: contracts/ArtYume/BuyARYMWithFiat.sol


pragma solidity ^0.8.20;

contract BuyFTCWithFiat is ReentrancyGuard {
    ...
    function addBackendWallet(address newWallet) external  {
        require(msg.sender == owner, "Caller is not the owner");
        backendWallets.push(newWallet);
    }


    function removeBackendWallet(address wallet) external {
        require(msg.sender == owner, "Caller is not the owner");
        for (uint i = 0; i < backendWallets.length; i++) {
    ...
    function send(address user, uint256 amount) external nonReentrant onlyBackendWallet{
        token.transfer(user, amount);
    }

    function outToken(uint256 amount) public onlyOwner {
